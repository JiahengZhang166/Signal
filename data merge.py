...
directory_path = 'C:/Users/zhang/Downloads/CFAR/10.26/data'
...
for i in range(len(option_files)):
    ...
    df_stock.columns = df_stock.columns.str.upper()
    df_stock['DATE'] = pd.to_datetime(df_stock['DLYCALDT'])


    df_stock['CAP'] = abs(df_stock['DLYPRC']) * df_stock['SHROUT']
    ...
    joined_df = pd.merge(df_stock, df_option_filtered, left_on=["TICKER", "DATE"],
                         right_on=["TICKER", "DATE"], how="inner")
    
    
    # Drop the redundant columns and keep only desired columns
    cleaned_df = joined_df[["TICKER", "DATE", "DLYRET", "CAP", "SLOPE"]]
...
final_df_sorted.to_csv('C:/Users/zhang/Downloads/CFAR/10.26/data/2018-2022.csv')